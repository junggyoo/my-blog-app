...
interface AdjascentPostCardProps {
  post: Post;
  type: 'prev' | 'next';
}
...
export default function AdjascentPostCard({
  post,
  type,
}: AdjascentPostCardProps) {
  ...
  return (
    <Link href={`/posts/${path}`} className="relative w-full bg-black max-h-56">
      ...
      <div className="group absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full flex justify-around items-center text-white px-8">
        {type === 'prev' && <FaArrowLeft className={ICON_STYLE} />}
        <div className="w-full text-center ">
          <h3 className="text-3xl font-bold">{title}</h3>
          <p className="font-bold">{description}</p>
        </div>
        {type === 'next' && <FaArrowRight className={ICON_STYLE} />}
      </div>
    </Link>
  );
}
